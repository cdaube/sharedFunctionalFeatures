...
def loadTrainData0th1st(projDir, costFunc, randomState=1, viewInvariant=False,frontalOnly=False):
    # define training and validation data files
    dataDir = projDir+'christoph_face_render_withAUs_20190730/'
    if not(viewInvariant):
        ...
        main0_data_txt = dataDir+'images_firstGen_ctrlSim_k1_355ModelsEquivalents/path/linksToImagesVINR.txt'
        main1_data_txt = dataDir+'images_firstGen_ctrlSim_k1/path/linksToImagesVINR.txt'
        colleagues0_data_txt = dataDir+'colleagueFaces355Models/meta/links_to_images_VINR.txt'
        colleagues1_data_txt = dataDir+'colleagueFacesSimStructk1/meta/links_to_images_VINR.txt'
    ...
    if frontalOnly:
        main_df = main_df[(main_df['yAnglex']==1) & (main_df['yAngley']==1)]
    ...
    if frontalOnly:
        colleague_df = colleague_df[(colleague_df['yAnglex']==1) & (colleague_df['yAngley']==1)]
...
def getActDetAE(model, thsBatch, destinDir, startLayer=0, batchNr=0):
    # get activations and store them
    for ll in range(startLayer,10):
