  ...
  fileNameSource <- paste("forwardModel_testMIB.mat", sep = "")
  ...
  names(thsTable) <- c("fold","coll","pps","fspc",
                  "texture","shape","triplet","classID","classMulti","VAE","mib")
  

  #if(FALSE) {
  # brms
  brmsModel <- brm(mib ~ 0 + (1|fspc:pps) + (1|fold:pps) + (1|coll:pps) +
                     texture + shape + triplet + classID + classMulti + VAE,
  ...
  writeMat(paste(rootDir,destinFolder,"extractedFit_forwardModelMIB.mat",sep = ""),extractedFit=extractedFit)
  # ... including names ...
  writeMat(paste(rootDir,destinFolder,"extractedFit_forwardModelMIB_names.mat",sep = ""),names=names(brmsFit))
  # ... and model
  save(brmsModel, file=paste(rootDir,destinFolder,"brmsModel_forwardModelMIB.rda",sep = ""))
  #}
  
  # hypotheses
  load(paste(rootDir,destinFolder,"brmsModel_forwardModelMIB.rda",sep = ""))
  ...
  featureSpaces <- c("texture","shape","triplet","classID","classMulti","VAE");
  ...
  writeMat(paste(rootDir,destinFolder,"hypotheses_forwardModelMIB.mat",sep = ""),evidenceRatios=evidenceRatios,pp=pp)
