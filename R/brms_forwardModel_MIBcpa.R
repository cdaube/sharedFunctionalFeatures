  ...
  fileNameSource <- paste("forwardModel_testMIBcpa.mat", sep = "")
  ...
  #if(FALSE) {
  # brms
  brmsModel <- brm(mib ~ 0 + (1|fold:coll) +
                     Texture + Shape + pixelPCA + ShapeANDTexture + ShapeANDpixelPCA +
                     Triplet_emb + ClassID_emb + ClassMulti_emb + AE_emb + viAE_emb + VAE_emb + beta10VAE_emb +
                     ShapeANDAE_emb + ShapeANDviAE_emb + ShapeANDbeta1VAE_emb + ShapeANDClassMulti_embANDbeta1VAE +
                     ShapeANDTextureANDAE_emb + ShapeANDTextureANDviAE_emb +
  ...
  png(file="M_forwardModelMIBcpa_chains.png",width=500,height=10000)
  plot(brmsModel, ask = FALSE, N = 50) # thank you Jack E. Taylor for making me aware of the N argument <3
  dev.off()

  png(file="M_forwardModelMIBcpa_ppCheck.png")
  pp_check(brmsModel)
  dev.off()
  ...
  writeMat(paste(rootDir,destinFolder,"extractedFit_forwardModelMIBcpa.mat",sep = ""),extractedFit=extractedFit)
  # ... including names ...
  writeMat(paste(rootDir,destinFolder,"extractedFit_forwardModelMIBcpa_names.mat",sep = ""),names=names(brmsFit))
  # ... and model
  save(brmsModel, file=paste(rootDir,destinFolder,"brmsModel_forwardModelMIBcpa.rda",sep = ""))
  #}
  
  # hypotheses
  load(paste(rootDir,destinFolder,"brmsModel_forwardModelMIBcpa.rda",sep = ""))
  ...
  writeMat(paste(rootDir,destinFolder,"hypotheses_forwardModelMIBcpa.mat",sep = ""),evidenceRatios=evidenceRatios,pp=pp)
